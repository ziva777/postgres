...
l1:
	...
	HeapTupleAndHeaderSetXmax(page, &tp, new_xmax);
			...
			TU_UpdateIndexes *update_indexes)
{
	...
	oldtup.t_self = *otid;
	HeapTupleCopyXidsFromPage(buffer, &oldtup, page);
...
l2:
	...
	if (newtup != heaptup)
	{
		 ...
		 */
		Assert(!IsToastRelation(relation));
		HeapTupleAndHeaderSetXmin(newpage, newtup, xid);
...
heap_index_delete_tuples(Relation rel, TM_IndexDeleteOp *delstate)
{
	...
	for (int i = 0; i < delstate->ndeltids; i++)
	{
		...
		for (;;)
		{
			...
			HeapTupleCopyXidsFromPage(buf, &htup, page);
...
freeze_single_heap_page(Relation relation, Buffer buffer)
{
	...
	struct VacuumCutoffs	vac_cutoffs = {0};
	VacuumParams			vac_params = {0};
	GlobalVisState		   *vistest;

	vacuum_get_cutoffs(relation, vac_params, &vac_cutoffs);

	vistest = GlobalVisTestFor(relation);

	{
		TransactionId	relfrozen_xid = vac_cutoffs.FreezeLimit;
		MultiXactId		relmin_mxid = vac_cutoffs.MultiXactCutoff;
		OffsetNumber	dummy_off_loc;

		PruneFreezeParams params = {.relation = relation,.buffer = buffer,
			.reason = PRUNE_ON_ACCESS,.options = HEAP_PAGE_PRUNE_FREEZE,
			.vistest = vistest,.cutoffs = &vac_cutoffs
		};

		heap_page_prune_and_freeze(&params, &presult, &dummy_off_loc,
								   &relfrozen_xid, &relmin_mxid, false);
	}
