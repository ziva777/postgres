...
freeze_single_heap_page(Relation relation, Buffer buffer)
{
	...
	heap_page_prune_and_freeze(relation, buffer, vistest, 0, NULL, &presult,
							   PRUNE_ON_ACCESS, &dummy_off_loc, NULL, NULL,
							   false);
	...
	if (nfrozen > 0)
	{
		...
		if (RelationNeedsWAL(relation))
		{
			...
			snapshotConflictHorizon = cutoffs.FreezeLimit;
			TransactionIdRetreatOrInvalid(&snapshotConflictHorizon);

			log_heap_prune_and_freeze(relation, buffer, snapshotConflictHorizon,
									  true, PRUNE_ON_ACCESS, frozen, nfrozen,
