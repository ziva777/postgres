...
freeze_single_heap_page(Relation relation, Buffer buffer)
{
	...
	TransactionId			relfrozen_xid;
	MultiXactId				relmin_mxid;

	vacuum_get_cutoffs(relation, params, &cutoffs);
	relfrozen_xid = cutoffs.FreezeLimit;
	relmin_mxid = cutoffs.MultiXactCutoff;

	vistest = GlobalVisTestFor(relation);

	heap_page_prune_and_freeze(relation, buffer, vistest,
							   HEAP_PAGE_PRUNE_FREEZE, &cutoffs, &presult,
							   PRUNE_ON_ACCESS, &dummy_off_loc,
