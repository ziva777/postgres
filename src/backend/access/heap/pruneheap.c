...
static void heap_prune_chain(Oid reloid, Page page, BlockNumber blockno,
							 OffsetNumber maxoff, OffsetNumber rootoffnum,
							 PruneState *prstate);
...
heap_page_prune_opt(Relation relation, Buffer buffer)
{
	 ...
	 *
	 * NOTE: Because this is an opportunistic function, it is typical to see
	 * inconsistent pages here. As a result, we couldn't tell whether it was a
	 * toast or heap page just from pd_special. So utilise a catalogue for this.
	 */
				  ...
				  OffsetNumber *off_loc)
{
	...
	for (int i = prstate->nroot_items - 1; i >= 0; i--)
	{
		...
		*off_loc = offnum;
		...
		heap_prune_chain(reloid, page, blockno, maxoff, offnum, prstate);
	}
						   ...
						   bool repairFragmentation)
{
	 ...
	 * bit.
	 */
	do_hint_prune = HeapPageGetPruneXid(page) != prstate.new_prune_xid ||
		PageIsFull(page);
	...
	if (do_prune || do_freeze)
	{
		...
		if (do_prune)
		{
			heap_page_prune_execute(buffer, false,
									...
									repairFragmentation,
									IsToastRelation(params->relation));
		}

		if (do_freeze)
			heap_freeze_prepared_tuples(params->relation, buffer,
 ...
 */
static void
heap_prune_chain(Oid reloid, Page page, BlockNumber blockno, OffsetNumber maxoff,
