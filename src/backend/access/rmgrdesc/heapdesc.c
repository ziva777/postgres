...
heap2_desc(StringInfo buf, XLogReaderState *record)
{
	...
	if (info == XLOG_HEAP2_PRUNE)
	{
		...
		appendStringInfo(buf, "snapshotConflictHorizon: %llu, nredirected: %u, ndead: %u, isCatalogRel: %c",
	...
	else if (info == XLOG_HEAP2_FREEZE_PAGE)
	{
		...
		appendStringInfo(buf, "snapshotConflictHorizon: %llu, nplans: %u, isCatalogRel: %c",
						 (unsigned long long) xlrec->snapshotConflictHorizon,
						 xlrec->nplans, xlrec->isCatalogRel ? 'T' : 'F');
