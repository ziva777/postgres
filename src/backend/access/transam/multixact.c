...
typedef struct MultiXactStateData
{
	 ...
	 * nextMulti counter just before first acquiring a shared lock in a given
	 ...
	 * we compute it (using nextMulti if none are valid).  Each backend is
...
MultiXactIdSetOldestMember(void)
{
	if (!MultiXactIdIsValid(OldestMemberMXactId[MyProcNumber]))
	{
		 ...
		 * fact we do: suppose we pick up nextMulti and then lose the CPU for
		 * a long time.  Someone else could advance nextMulti, and then
		 * another someone else could compute an OldestVisibleMXactId that
		 * would be after the value we are going to store when we get control
		 * back.  Which would be wrong.
		 *
		 * Note that a shared lock is sufficient, because it's enough to stop
		 * someone from advancing nextMulti; and nobody else could be trying
		...
		/*
		 * We have to beware of the possibility that nextMulti is in the
		...
		OldestMemberMXactId[MyProcNumber] = nextMulti;

		LWLockRelease(MultiXactGenLock);

		debug_elog4(DEBUG2, "MultiXact: setting OldestMember[%d] = %u",
					MyProcNumber, nextMulti);
	}
}
 ...
 *
 * The value to set is the oldest of nextMulti and all the valid per-backend
...
MultiXactIdSetOldestVisible(void)
{
	if (!MultiXactIdIsValid(OldestVisibleMXactId[MyProcNumber]))
	{
		 ...
		 * must be sure to store a valid value in our array entry.
		 */
		oldestMulti = MxidFromFullMultiXactId(MultiXactState->nextMulti);
		if (oldestMulti < FirstMultiXactId)
			oldestMulti = FirstMultiXactId;
		...
		for (i = 0; i < MaxOldestSlot; i++)
		{
			...
			if (MultiXactIdIsValid(thisoldest) &&
				MultiXactIdPrecedes(thisoldest, oldestMulti))
				oldestMulti = thisoldest;
		}

		OldestVisibleMXactId[MyProcNumber] = oldestMulti;

		LWLockRelease(MultiXactGenLock);

		debug_elog4(DEBUG2, "MultiXact: setting OldestVisible[%d] = %u",
					MyProcNumber, oldestMulti);
	}
}
...
ReadNextMultiXactId(void)
{
	...
	LWLockAcquire(MultiXactGenLock, LW_SHARED);
	mxid = MxidFromFullMultiXactId(MultiXactState->nextMulti);
	LWLockRelease(MultiXactGenLock);
...
{
	LWLockAcquire(MultiXactGenLock, LW_SHARED);
	*oldest = MxidFromFullMultiXactId(MultiXactState->oldestMulti);
	*next = MxidFromFullMultiXactId(MultiXactState->nextMulti);
	LWLockRelease(MultiXactGenLock);
...
#define OFFSET_WARN_SEGMENTS	20
	 ...
	 * start offset after advancing nextMulti would effectively corrupt the
	 * previous MultiXact.)
	 */
	START_CRIT_SECTION();
	 ...
	 * the next iteration.  But note that nextMulti may be InvalidMultiXactId
...
/*
 * Compute the FullMultiXactId for the given MultiXactId.
 *
 * The wrap logic is safe here because the span of active multixact cannot
 * exceed one epoch at any given time.
 */
static FullMultiXactId
AdjustToFullMultiXactId(MultiXactId multi)
{
 ...
 * range, that is, greater to or equal than oldestMulti, and less than
 * nextMulti.  Otherwise, an error is raised.
 *
					  ...
					  bool from_pgupgrade, bool isLockOnly)
{
	 ...
	 *
	 * An ID older than MultiXactState->oldestMulti cannot possibly be
	 * useful; it has already been removed, or will be removed shortly, by
	 * truncation.  If one is passed, an error is raised.
	 *
	 * Also, an ID >= nextMulti shouldn't ever be seen here; if it is seen, it
	 * implies undetected ID wraparound has occurred.  This raises a hard
	 * error.
	 *
	 * Shared lock is enough here since we aren't modifying any global state.
	 * Acquire it just long enough to grab the current counter values.  We may
	 * need both nextMulti and nextOffset; see below.
	 */
	LWLockAcquire(MultiXactGenLock, LW_SHARED);
	...
	if (MultiXactIdPrecedes(multi, oldestMulti))
		ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
				 errmsg("MultiXactId %u does no longer exist -- apparent wraparound",
						multi)));

	if (!MultiXactIdPrecedes(multi, nextMulti))
		ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
...
retry:
	...
	else
	{
		...
		if (tmpMulti < FirstMultiXactId)
			tmpMulti = FirstMultiXactId;

		prev_pageno = pageno;

		pageno = MultiXactIdToOffsetPage(tmpMulti);
		entryno = MultiXactIdToOffsetEntry(tmpMulti);
		...
		if (pageno != prev_pageno)
		{
			...
			slotno = SimpleLruReadPage(MultiXactOffsetCtl, pageno, true, tmpMulti);
		}
...
MaybeExtendOffsetSlru(void)
{
	...
	pageno = MultiXactIdToOffsetPage(MxidFromFullMultiXactId(MultiXactState->nextMulti));
	lock = SimpleLruGetBankLock(MultiXactOffsetCtl, pageno);
 ...
 *
 * StartupXLOG has already established nextMulti/nextOffset by calling
...
{
	MultiXactId multi = MxidFromFullMultiXactId(MultiXactState->nextMulti);
...
TrimMultiXact(void)
{
	 ...
	 */
	pageno = MultiXactIdToOffsetPage(nextMulti);
	pg_atomic_write_u64(&MultiXactOffsetCtl->shared->latest_page_number,
						pageno);
	 ...
	 * rule "write xlog before data," nextMulti successors may carry obsolete,
	 * nonzero offset values.  Zero those so case 2 of GetMultiXactIdMembers()
	 * operates normally.
	 */
	entryno = MultiXactIdToOffsetEntry(nextMulti);
	if (entryno != 0)
	{
		int			slotno;
		MultiXactOffset *offptr;
		...
		LWLockAcquire(lock, LW_EXCLUSIVE);
		slotno = SimpleLruReadPage(MultiXactOffsetCtl, pageno, true, nextMulti);
		offptr = (MultiXactOffset *) MultiXactOffsetCtl->shared->page_buffer[slotno];
		offptr += entryno;
					  ...
					  MultiXactOffset nextMultiOffset)
{
	 ...
	 * routine we determine the MultiXactState->nextMulti value that
						...
						bool is_startup)
{
	...
	curMulti = MxidFromFullMultiXactId(MultiXactState->nextMulti);
	LWLockRelease(MultiXactGenLock);
...
/*
 * Update our oldestMulti value, but only if it's more recent than what
 * we had.
 *
...
GetOldestMultiXactId(void)
{
	 ...
	 * OldestVisibleMXactId[] entries, or nextMulti if none are valid.
	 */
	LWLockAcquire(MultiXactGenLock, LW_SHARED);
...
SetOffsetVacuumLimit(bool is_startup)
{
	 ...
	 * case: if there are no multixacts in existence at all, oldestMulti
	...
	else
	{
		 ...
		 */
		oldestOffsetKnown =
			find_multixact_start(oldestMulti, &oldestOffset);
...
ReadMultiXactCounts(uint32 *multixacts, MultiXactOffset *members)
{
	...
	nextOffset = MultiXactState->nextOffset;
	oldestMulti = MxidFromFullMultiXactId(MultiXactState->oldestMulti);
	nextMultiXactId = MxidFromFullMultiXactId(MultiXactState->nextMulti);
	...
	*multixacts = nextMultiXactId - oldestMulti;
	return true;
}
...
TruncateMultiXact(MultiXactId newOldestMulti, Oid newOldestMultiDB)
{
	...
	FullMultiXactId newOldestFullMulti;
	...
	nextOffset = MultiXactState->nextOffset;
	oldestMulti = MultiXactState->oldestMulti;
	newOldestFullMulti = AdjustToFullMultiXactId(newOldestMulti);
	LWLockRelease(MultiXactGenLock);
	Assert(FullMultiXactIdIsValid(oldestMulti));
	 ...
	 */
	if (MultiXactIdPrecedesOrEquals(newOldestMulti,
									MxidFromFullMultiXactId(oldestMulti)))
	{
		LWLockRelease(MultiXactTruncationLock);
		return;
	 ...
	 *
	 * When nextMulti is less than one segment away from multiWrapLimit,
	...
	if (MultiXactIdPrecedes(MxidFromFullMultiXactId(oldestMulti), earliest))
	{
		LWLockRelease(MultiXactTruncationLock);
		return;
	}
	 ...
	 */
	if (FullMultiXactIdEquals(oldestMulti, nextMulti))
	{
		/* there are NO MultiXacts */
		oldestOffset = nextOffset;
	}
	else if (!find_multixact_start(MxidFromFullMultiXactId(oldestMulti),
								   &oldestOffset))
	{
		ereport(LOG,
				(errmsg("oldest MultiXact %u:%u not found, earliest MultiXact %u, skipping truncation",
						EpochFromFullMultiXactId(oldestMulti),
						MxidFromFullMultiXactId(oldestMulti),
						earliest)));
		LWLockRelease(MultiXactTruncationLock);
	 ...
	 */
	if (FullMultiXactIdEquals(newOldestFullMulti, nextMulti))
	{
		/* there are NO MultiXacts */
	...
	elog(DEBUG1, "performing multixact truncation: "
		 "offsets [%u:%u, %u:%u), offsets segments [%llx, %llx), "
		 "members [%u, %u), members segments [%llx, %llx)",
		 EpochFromFullMultiXactId(oldestMulti),
		 MxidFromFullMultiXactId(oldestMulti),
		 EpochFromFullMultiXactId(newOldestFullMulti),
		 MxidFromFullMultiXactId(newOldestFullMulti),
		 (unsigned long long) MultiXactIdToOffsetSegment(MxidFromFullMultiXactId(oldestMulti)),
	...
	WriteMTruncateXlogRec(newOldestMultiDB,
						  oldestMulti, newOldestFullMulti,
						  oldestOffset, newOldestOffset);
	 ...
	 */
	LWLockAcquire(MultiXactGenLock, LW_EXCLUSIVE);
	MultiXactState->oldestMulti = newOldestFullMulti;
	...
	/* Then offsets */
	PerformOffsetsTruncation(MxidFromFullMultiXactId(oldestMulti),
							 newOldestMulti);
...
WriteMTruncateXlogRec(Oid oldestMultiDB,
					  FullMultiXactId startTruncOff, FullMultiXactId endTruncOff,
...
multixact_redo(XLogReaderState *record)
{
	...
	else if (info == XLOG_MULTIXACT_TRUNCATE_ID)
	{
		...
		elog(DEBUG1, "replaying multixact truncation: "
			 ...
			 (unsigned long long) MultiXactIdToOffsetSegment(MxidFromFullMultiXactId(xlrec.startTruncOff)),
			 (unsigned long long) MultiXactIdToOffsetSegment(MxidFromFullMultiXactId(xlrec.endTruncOff)),
		 ...
		 * SimpleLruTruncate.
		 */
		pageno = MultiXactIdToOffsetPage(MxidFromFullMultiXactId(xlrec.endTruncOff));
		pg_atomic_write_u64(&MultiXactOffsetCtl->shared->latest_page_number,
							pageno);
		PerformOffsetsTruncation(MxidFromFullMultiXactId(xlrec.startTruncOff),
								 MxidFromFullMultiXactId(xlrec.endTruncOff));
