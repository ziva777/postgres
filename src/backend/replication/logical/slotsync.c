							  ...
							  slot->data.catalog_xmin))
	{
		...
		ereport(slot->data.persistency == RS_TEMPORARY ? LOG : DEBUG1,
				errmsg("could not synchronize replication slot \"%s\"",
					   remote_slot->name),
				errdetail("Synchronization could lead to data loss, because the remote slot needs WAL at LSN %X/%08X and catalog xmin %llu, but the standby has LSN %X/%X and catalog xmin %llu.",
